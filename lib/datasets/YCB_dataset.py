...
from lib.utils.data_utils import read_pickle, YCBImageDB, read_rgb_np, read_mask_np, YCBModelDB
import os
import torch
...
def read_mask_ycb(mask_path):
    mask = Image.open(mask_path)
    mask_seg = np.array(mask)
    return mask_seg
...
class VotingType:
    ...
    @staticmethod
    def get_data_pts_2d_YCB(data):
        cen = data['centers'].copy()
        far = data['farthests'].copy()
        hcoords = np.concatenate([far, cen], 0)
        hcoords = np.concatenate([hcoords, np.ones([hcoords.shape[0], 1], np.float32)], 1)
        return hcoords
    ...
    def get_pts_3d(vote_type,class_type):
        ...
        if vote_type==VotingType.BB8C:
            ...
            points_3d = np.concatenate([points_3d,ycb_db.get_center_3d(class_type)[None,:]],0)
        elif vote_type==VotingType.BB8S:
...
class YCBDatasetRealAug(Dataset):
    def __init__(self, imagedb, class_type, background_list, data_prefix=cfg.YCB, vote_type=VotingType.Farthest,
                 augment=False, cfg=default_aug_cfg, background_mask_out=False, use_intrinsic=True,
                 use_motion=False):
        ...
        self.cfg=cfg
        self.ycb_dir = data_prefix
        self.cls_type = class_type
        ...
        self.all_classnames = np.loadtxt(os.path.join(self.ycb_dir, 'image_sets/classes.txt'), dtype=np.str).tolist()
        self.cls_name_id = {self.all_classnames[i]:i for i in range(len(self.all_classnames))}
    ...
    def __getitem__(self, index_tuple):
        ...
        rgb_path = os.path.join(self.data_prefix,self.imagedb[index]['rgb_path'])
        mask_path = os.path.join(self.data_prefix,self.imagedb[index]['mask_path'])
    ...
    def augmentation(self, img, mask, hcoords, height, width):
        ...
        if self.cfg['mask'] and np.random.random() < 0.5 and foreground > 4000:
...
class ImageSizeBatchSampler(Sampler):
    def __init__(self, sampler, batch_size, drop_last=True, cfg=default_aug_cfg):
