...
class EstimateEncoder(nn.Module):
    def __init__(self, ver_dim, seg_dim, fcdim=256, s8dim=128, s4dim=64, s2dim=32, raw_dim=32, pre_trained=True):
        ...
        # x8s->128
        self.conv8s=nn.Sequential(
            nn.Conv2d(384, s8dim, 3, 1, 1, bias=False),
            nn.BatchNorm2d(s8dim),
            nn.LeakyReLU(0.1,True)
        )
        self.up8sto4s=nn.UpsamplingBilinear2d(scale_factor=2)
...
class RefineNet(nn.Module):
    def __init__(self, ver_dim, seg_dim, fcdim=256, s8dim=128, s4dim=64, s2dim=32, raw_dim=32):
        ...
        self.resnet = Resnet18_8s(ver_dim, seg_dim, fcdim, s8dim, s4dim, s2dim, raw_dim)
    ...
    def forward(self, img, vertexEst):
        x2s, imageFeatures = self.resnet(img)
        estFeatures = self.estimateEncoder(vertexEst)
        fm = torch.cat([imageFeatures,estFeatures],1)
        fm = self.up4sto2s(fm)
        fm = self.conv2s(torch.cat([fm, x2s], 1))
        fm = self.up2storaw(fm)
        x = self.convraw(torch.cat([fm, img], 1))
        seg_pred = x[:, :self.resnet.seg_dim, :, :]
        ver_pred = x[:, self.resnet.seg_dim:, :, :]
