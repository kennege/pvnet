...
from lib.utils.net_utils import AverageMeter, Recorder, smooth_l1_loss, smooth_l1_loss_v1, smooth_l1_loss_v2, \
    center_distance_l2, center_distance_l2_v2, center_voting_loss_v1, \
...
train_cfg['model_name']='{}_linemod_train_noReal_pbr'.format(args.linemod_cls,train_cfg['model_name'])
...
ver_dist_loss_rec = AverageMeter()
voting_loss_rec = AverageMeter()
precision_rec = AverageMeter()
recall_rec = AverageMeter()
recs=[seg_loss_rec,ver_loss_rec, ver_dist_loss_rec, voting_loss_rec, precision_rec,recall_rec]
recs_names=['scalar/seg','scalar/ver', 'scalar/vdist', 'scalar/voting', 'scalar/precision','scalar/recall']
...
def val(net, dataloader, epoch, val_prefix='val', use_camera_intrinsic=False, use_motion=False):
    ...
    for idx, data in enumerate(dataloader):
        ...
        with torch.no_grad():
            seg_pred, vertex_pred, loss_seg, loss_vertex, loss_p2l, loss_voting, precision, recall = net(image, mask, vertex, vertex_weights, corner_target)

            # print(seg_pred.shape)
            # seg_mask = seg_pred[0,0,:]>seg_pred[0,1,:]
            # seg_mask = seg_mask.cpu().numpy()
            # seg_mask = np.asarray(seg_mask*255, dtype=np.uint8)
            # filename = "/media/yu/xin/DHV/data/visual/{:d}.png".format(idx)
            # cv2.imwrite(filename, seg_mask)

            loss_seg, loss_vertex, loss_p2l, loss_voting, precision, recall=[torch.mean(val) for val in (loss_seg, loss_vertex, loss_p2l, loss_voting, precision, recall)]
            ...
            vals=[loss_seg,loss_vertex,loss_p2l, loss_voting, precision,recall]
...
def test_net():
    ...
    #     train_batch_sampler = ImageSizeBatchSampler(train_sampler, train_cfg['train_batch_size'], True, cfg=train_cfg['aug_cfg'])
    #     train_loader = DataLoader(train_set, batch_sampler=train_batch_sampler, pin_memory=False, num_workers=8)
    #
    #     val_db=image_db.test_real_set
    ...
    #     val_batch_sampler = ImageSizeBatchSampler(val_sampler, train_cfg['test_batch_size'], True, cfg=train_cfg['aug_cfg'])
    #     val_loader = DataLoader(val_set, batch_sampler=val_batch_sampler, pin_memory=False, num_workers=8)
    #
    #     if args.linemod_cls in cfg.occ_linemod_cls_names:
    #         occ_image_db=OcclusionLineModImageDB(args.linemod_cls)
    #         occ_val_db=occ_image_db.test_real_set[:len(occ_image_db.test_real_set)]
    ...
    #         occ_val_loader = DataLoader(occ_val_set, batch_sampler=occ_val_batch_sampler, pin_memory=False, num_workers=8)
    #
